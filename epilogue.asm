...
fmt_incorrect_arity_simple:
        db `!!! Expected %ld arguments, but given %ld n\0`
...
L_code_ptr_bin_apply:
        ...
        mov r8, qword [rsp + 8 * (3 + 0)]       ; param 0 = proc = closure
        assert_closure(r8)
        mov rcx, qword [rsp + 8 * (3 + 1)]      ; param 1 = argList
        assert_pair(rcx)
        mov r10, qword [rsp]                    ; ret_addr

        lea rsp, [rsp + 8 * 5]                  ; rsp go back to before

;         mov rbx, sob_nil
;  .loop_push_args1:
;         cmp byte [rcx], T_pair
;         ;mov rbx, byte [rdi]
;         jne .end_loop_push_args1
;         mov rdi, (1 + 8 + 8)
;         call malloc
;         mov SOB_PAIR_CDR(rax), rbx
;         mov rdx, SOB_PAIR_CAR(rcx)
...
;         mov rcx, rax
        ...
        mov rsi, 0      ; counter of argc
.loop_push_args:
        cmp byte [rcx], T_pair
        ;mov rbx, byte [rdi]
        jne .end_loop_push_args
        mov rax, SOB_PAIR_CAR(rcx)
        push rax
        mov rcx, SOB_PAIR_CDR(rcx)
        inc rsi
        jmp .loop_push_args
...
L_code_ptr_cons:
        ...
        mov byte [rax], T_pair
        mov rbx, PARAM(0)
        mov SOB_PAIR_CAR(rax), rbx ; cons
        mov rbx, PARAM(1)
        mov SOB_PAIR_CDR(rax), rbx ; cons
        LEAVE
