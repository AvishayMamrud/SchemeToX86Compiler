...
exception X_not_yet_implemented1;;
exception X_not_yet_implemented2;;
exception X_not_yet_implemented3;;
exception X_not_yet_implemented4;;
exception X_not_yet_implemented5;;
exception X_not_yet_implemented6;;
exception X_not_yet_implemented7;;
exception X_not_yet_implemented8;;
exception X_not_yet_implemented9;;
exception X_not_yet_implemented10;;
exception X_not_yet_implemented11;;
exception X_not_yet_implemented12;;
exception X_this_should_not_happen of string;;
exception Ein_gafrur;;
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let unsymbolify_var = function
    | ScmSymbol var -> var
    | x -> raise (X_syntax ("not a symbol "^( sprint_sexpr "%a" x)));;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmSymbol var ->
       ...
       then raise (X_syntax ("Variable cannot be a reserved word "^var))
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
      let ribs_list = (match (scheme_list_to_ocaml ribs) with
                        | lst, _ -> lst) in
      let vars = (List.map (fun (rib) -> match rib with 
                                          | ScmPair (ScmSymbol varName, value) -> (ScmSymbol varName)) ribs_list) in
         let vals = List.map (fun (rib) -> match rib with 
                                          | ScmPair (ScmSymbol varName, ScmPair (value, ScmNil)) -> value) ribs_list in
      tag_parse (ScmPair (ScmPair (ScmSymbol "lambda", ScmPair (( Reader.scheme_sexpr_list_of_sexpr_list vars), exprs)), ( Reader.scheme_sexpr_list_of_sexpr_list vals)))
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      ...
      let vars = List.map (fun (rib) -> match rib with 
                                          | ScmPair( ScmSymbol varName, value) -> (ScmSymbol varName)) ribs_list in
      let bindings =  Reader.scheme_sexpr_list_of_sexpr_list (List.map (fun (sym_var) -> match sym_var with 
            | (ScmSymbol varName) -> (ScmPair (sym_var, ScmPair (      ScmPair(ScmSymbol "quote",                      ScmPair(   ScmSymbol "whatever"  , ScmNil)  )                 , ScmNil)))) vars) in
      let vals = List.map (fun (rib) -> match rib with 
            | ScmPair (ScmSymbol varName, ScmPair (value, ScmNil)) -> value) ribs_list in
      let set_list = (List.map2 (fun curr_var curr_val -> ScmPair (ScmSymbol "set!", ScmPair (curr_var, ScmPair (curr_val, ScmNil)))) vars vals) in
      let exprs_ocaml_style = match (scheme_list_to_ocaml exprs)with 
                              | (expr_lst, ScmNil) -> expr_lst
                              | (expr_lst, other_expr) -> expr_lst @ [other_expr] in
      tag_parse (ScmPair (ScmSymbol "let", ScmPair (bindings, (ScmPair (ScmPair (ScmSymbol "begin", ( Reader.scheme_sexpr_list_of_sexpr_list (set_list @ exprs_ocaml_style))), ScmNil)))))
   ...
   | ScmPair (proc, args) ->
      let proc =
         (match proc with
          | ScmSymbol var ->
             if (is_reserved_word var)
             then raise (X_syntax ("reserved word in proc position "^var))
             else proc
          | proc -> proc) in
...
let annotate_tail_calls = 
  ...
  let cross_product as' bs' =
   ...
   let rec get_env_origin env =
      match env with
         | []->[]
         | [rib]->rib
         | _::rest_ribs-> get_env_origin rest_ribs

   let compare_origins env1 env2 =
      let origin1 = get_env_origin env1 in
         let origin2 = get_env_origin env2 in
            if (origin1 = [] || origin2 = []) then false 
            else (not (origin1 == origin2))

   let check_rules_for_box a=
      match a with
         | ((Var' (_, Param _),_), (Var'(_,Param _),_)) -> false
         | ((Var' (_,Param _),_), (Var'(_,Bound (_,_)),_)) -> true
         | ((Var' (_,Bound (_,_)), _), (Var'(_,Param _),_)) -> true
         | ((Var' (_,Bound (_,_)), env1), (Var'(_,Bound(_,_)),env2)) -> compare_origins env1 env2
         | _ -> false
                 
   let rec check_cross cross=
      match cross with
         | [] -> false
         | pair1::rest-> if (check_rules_for_box pair1) then true else (check_cross rest)

  let should_box_var name expr params = 
      let (env_read, env_write) = find_reads_and_writes name expr params in
         let cross = cross_product env_read env_write in
            check_cross cross;;
